...
def my_models_2(x,y,x_list,log_en,external_test_data=0,ext_x=[],ext_y=[]):
    ...
    def split_train_test(x, y, x_list, window_size):
        ...
        # Augmented x data
            ...
            for i in class_list:       # class별로 'idle', 'suit1', ....
                ...
                n_train = int(0.8 * len(ii[idx_class]))
                n_valid = int(0.1 * len(ii[idx_class]))
                n_test  = int(0.1 * len(ii[idx_class]))
...
#                 x_test,  y_test  = x[idx_class][n_train+n_valid:], y[idx_class][n_train+n_valid:]
    ...
    ## 모형 설계
    >> - conv1D (input_size, output_size, kernel_size)
 """
    ...
    # 모델 검증
    Acc_1D_CNN_valid = test_model(model, test_loader)

 

    """## 별도의 Test Data로 검증"""

    x_list = [ext_x]   # 별도의 test 에서는 augment 하지 않으므로, x_list는 원본 external x만
    ...
    # 모델 검증 (새로운 싯점 데이터 기준 )
    Acc_1D_CNN = test_model(model, train_loader)
